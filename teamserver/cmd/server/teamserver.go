...
func findBuildTool(configured, fallback, label string) (string, bool) {
	if len(configured) > 0 {
		if _, err := os.Stat(configured); err == nil {
			return configured, true
		} else if !os.IsNotExist(err) {
			logger.SetStdOut(os.Stderr)
			logger.Error(fmt.Sprintf("%s path is not usable: %s: %v", label, configured, err))
			return "", false
		}
		...
		if base := filepath.Base(configured); base != configured {
		...
		logger.SetStdOut(os.Stderr)
		logger.Error(fmt.Sprintf("%s path doesn't exist and %s was not found on PATH: %s", label, fallback, configured))
		return "", false
	}
	...
	if err != nil {
		logger.SetStdOut(os.Stderr)
		logger.Error(fmt.Sprintf("Couldn't find %s: %v", label, err))
		return "", false
	}
...
func (t *Teamserver) FindSystemPackages() bool {
	...
	if t.Profile.Config.Server.Build != nil {
		compiler64 = t.Profile.Config.Server.Build.Compiler64
		compiler86 = t.Profile.Config.Server.Build.Compiler86
		nasm = t.Profile.Config.Server.Build.Nasm
	}

	if value, ok := findBuildTool(compiler64, "x86_64-w64-mingw32-gcc", "x64 mingw compiler"); ok {
		t.Settings.Compiler64 = value
	} else {
		return false
	}

	if value, ok := findBuildTool(compiler86, "i686-w64-mingw32-gcc", "x86 mingw compiler"); ok {
		t.Settings.Compiler32 = value
	} else {
		return false
	}

	if value, ok := findBuildTool(nasm, "nasm", "nasm"); ok {
		t.Settings.Nasm = value
	} else {
		return false
	}
