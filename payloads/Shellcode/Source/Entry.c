...
SEC( text, B ) VOID Entry( VOID )
{
    ...
    if ( NT_SUCCESS( Instance.Win32.NtAllocateVirtualMemory( NtCurrentProcess(), &KVirtualMemory, 0, &KMemSize, MEM_COMMIT, PAGE_READWRITE ) ) )
    {
        ...
        KaynArgs.DemonSize = ( DWORD ) KMemSize;
        ...
        if ( ImageDir->VirtualAddress )
            KaynLdrReloc( KVirtualMemory, ( ULONG_PTR ) NtHeaders->OptionalHeader.ImageBase, C_PTR( KVirtualMemory + ImageDir->VirtualAddress ), KHdrSize );
...
VOID KaynLdrReloc( PVOID KaynImage, ULONG_PTR ImageBase, PVOID BaseRelocDir, DWORD KHdrSize )
{
    PIMAGE_BASE_RELOCATION  pImageBR = C_PTR( BaseRelocDir - KHdrSize );
    LPVOID                  OffsetIB = C_PTR( ( U_PTR( KaynImage ) - KHdrSize ) - ImageBase );
